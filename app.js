...
const DB = "mongodb+srv://Decostar:decostar@cluster0.hwjrpf2.mongodb.net/journal?retryWrites=true&w=majority";
...
app.post( "/", function(req, res )
{
  ...
  Adiary.findOne( {username :name , pswd :mail } , {} , function(request , response)
  {
    if(val=="login")
    {
      ...
      if(response)
      {
        req.flash("message" , "need to login .... Already have account ");
        res.redirect("/");
      }else{
          const temp = [] ;
          const dd = new Adiary({
...
//   Adiary.findOne( { email : mail , pswd : pwd } , function(err ,arr  )
// {
//   if(arr===null)
//   {
...
app.post("/edit" , function(req , res )
{
  ...
  Adiary.findOne( {username :name , pswd :mail } , {} , function(request , response) {
                  ...
                  response.diary.forEach( function( ele1 , ele2 )
                {
                  if( ele1._id == ID )
                  {
                    res.render(  "compose" , { heading : ele1.title , middle: ele1.body,    ID : ID  } ) ;
                  }
                })
